...
`coolms/rql` is a **standalone library**. It does not require `coolms/core`
and it is useful without CoolMS, so its version number answers to its own API
and nothing else.

⚠️ **That includes the major.** `coolms/rql 2.0.0` means *this package* broke
